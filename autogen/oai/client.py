...
class OpenAIWrapper:
    ...
    async def a_create(self, **config: Any) -> ChatCompletion:
        """Make a completion for a given config using openai's clients.
        Besides the kwargs allowed in openai's client, we allow the following additional kwargs.
        The config in each client will be overridden by the config.
            ...
            if cache_client is not None:
                with cache_client as cache:
                    ...
                    if response is not None:
                        ...
                        continue  # filter is not passed; try the next config
            try:
                response = await self._a_completions_create(
                    client, params, callback_config=callback_config)
            except APIError as err:
                error_code = getattr(err, "code", None)
                if error_code == "content_filter":
    ...
    def _completions_create(self, client: OpenAI, params: Dict[str, Any], **kwargs) -> ChatCompletion:
        ...
        if params.get("stream", False) and "messages" in params:
            ...
            if callback:
                callback.on_llm_start(**callback_params)
            ...
            try:
                ...
                for chunk in completions.create(**params):
                    if chunk.choices:
                        ...
                        for choice in chunk.choices:
                            ...
                            if callback:
                                callback.on_llm_new_token(token=content,
                                                          **callback_params)
                ...
                if callback:
                    callback.on_llm_error(**callback_params, error=e)
                raise e
            finally:
                if callback:
                    callback.on_llm_end(**callback_params,
                                              token_usage=token_usage)
        else:
            # If streaming is not enabled, send a regular chat
            # completion request
            params = params.copy()
            params["stream"] = False
    ...
    async def _a_completions_create(self, client: OpenAI, params: Dict[str, Any], **kwargs) -> ChatCompletion:
        ...
        if params.get("stream", False) and "messages" in params:
            ...
            # and process the response in chunks
            if callback:
                await callback.on_a_llm_start(**callback_params)
            ...
            try:
                ...
                for chunk in completions.create(**params):
                    if chunk.choices:
                        ...
                        for choice in chunk.choices:
                            ...
                            if callback:
                                await callback.on_a_llm_new_token(
                                    token=content,
                                    **callback_params
                                )
                ...
                if callback:
                    await callback.on_a_llm_error(**callback_params, error=e)
                raise e
            finally:
                if callback:
                   await callback.on_a_llm_end(**callback_params,
                                               token_usage=token_usage)
        else:
            # If streaming is not enabled, send a regular chat
            # completion request
